...
def test_dockerfile_image_pinning():
    ...
    for line_num, from_line in from_lines:
        ...
        if re.search(sha256_pattern, from_line):
            print(f"   ✅ SHA256-pinned image detected")
            pinned_count += 1
        else:
            print(f"   ❌ Image NOT pinned with SHA256 digest")
    
    if pinned_count == total_count:
        print(f"\n✅ Container security fix verified:")
        print(f"   ✅ All {total_count} base images are SHA256-pinned")
        print(f"   ✅ Supply chain attack protection enabled")
        print(f"   ✅ Immutable image references established")
        return True
    else:
        print(f"\n❌ Container security issue detected:")
        print(f"   ❌ {total_count - pinned_count} of {total_count} images are NOT pinned")
        print(f"   ⚠️  Supply chain attack vulnerability remains")
        return False
...
def test_specific_python_image_pin():
    ...
    expected_digest = "sha256:8df0e8faf75b3c17ac33dc90d76787bbbcae142679e11da8c6f16afae5605ea7"
    
    if expected_digest in content:
        print(f"✅ Expected Python image digest found: {expected_digest}")
        return True
    else:
        print(f"❌ Expected digest not found in Dockerfile")
