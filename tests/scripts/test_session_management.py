...
class TestSessionContextManager:
    ...
    def setup_method(self):
        ...
        mock_cl.user_session.get.side_effect = lambda key, default=None: self.mock_session_data.get(key, default)
        mock_cl.user_session.set.side_effect = lambda key, value: self.mock_session_data.update({key: value})
    ...
    def test_set_current_cwe_success(self):
        ...
        # Mock session ID
        with patch.object(self.session_manager, '_get_session_id', return_value='test-session-123'):
            success = self.session_manager.set_current_cwe('CWE-79', {'description': 'XSS vulnerability'})
    ...
    def test_session_isolation(self):
        ...
        # Session 1
        with patch.object(self.session_manager, '_get_session_id', return_value='session-1'):
            session_1_data = {}
            mock_cl.user_session.get.side_effect = lambda key, default=None: session_1_data.get(key, default)
            mock_cl.user_session.set.side_effect = lambda key, value: session_1_data.update({key: value})
            
            self.session_manager.set_current_cwe('CWE-79')
        
        # Session 2
        with patch.object(self.session_manager, '_get_session_id', return_value='session-2'):
            session_2_data = {}
            mock_cl.user_session.get.side_effect = lambda key, default=None: session_2_data.get(key, default)
            mock_cl.user_session.set.side_effect = lambda key, value: session_2_data.update({key: value})
    ...
    def test_thread_safety(self):
        ...
        def worker(session_id, cwe_id):
            with patch.object(self.session_manager, '_get_session_id', return_value=session_id):
                # Each thread works with its own mock session data
                thread_data = {}
                with patch.object(mock_cl.user_session, 'get', side_effect=lambda key, default=None: thread_data.get(key, default)):
                    with patch.object(mock_cl.user_session, 'set', side_effect=lambda key, value: thread_data.update({key: value})):
...
class TestSessionSecurityValidator:
    ...
    def test_validate_session_isolation_valid_session(self):
        ...
        with patch.object(self.security_validator, '_verify_context_isolation', return_value=True):
    ...
    def test_detect_context_contamination_clean(self):
        ...
        clean_context = {
            'current_cwe': {'cwe_id': 'CWE-79', 'timestamp': datetime.utcnow().isoformat()},
            'history': []
        }
        
        contamination = self.security_validator.detect_context_contamination(session_id, clean_context)
    ...
    def test_detect_context_contamination_suspicious(self):
        ...
        contaminated_context = {
            'session_refs': ['12345678-1234-1234-1234-123456789abc', '87654321-4321-4321-4321-cba987654321'],
            'data': 'some data'
        }
        
        with patch.object(self.security_validator, '_check_multiple_session_ids', return_value=True):
            contamination = self.security_validator.detect_context_contamination(session_id, contaminated_context)
    ...
    def test_validate_session_boundaries(self):
        ...
        with patch.object(self.security_validator, 'validate_session_isolation', return_value=True):
            with patch.object(self.security_validator, 'detect_context_contamination', return_value=False):
                with patch.object(self.security_validator, '_validate_session_metadata', return_value=True):
                    ...
                    assert result['security_score'] > 90.0  # High score for clean session
    ...
    def test_validate_session_boundaries_with_issues(self):
        ...
        with patch.object(self.security_validator, 'validate_session_isolation', return_value=False):
            with patch.object(self.security_validator, 'detect_context_contamination', return_value=True):
    ...
    def test_timestamp_validation(self):
        ...
        result = self.security_validator._check_timestamp_anomalies(session_id, old_context)
        ...
        result = self.security_validator._check_timestamp_anomalies(session_id, recent_context)
...
class TestSessionIntegration:
    ...
    def test_session_workflow_integration(self):
        ...
        with patch.object(self.session_manager, '_get_session_id', return_value=session_id):
            ...
            mock_cl.user_session.set.side_effect = lambda key, value: session_data.update({key: value})
            
            # 1. Validate session security
            security_valid = self.security_validator.validate_session_isolation(session_id)
            assert security_valid is True
            
            # 2. Set context
            context_set = self.session_manager.set_current_cwe('CWE-79', {'name': 'XSS'})
            assert context_set is True
            
            # 3. Validate no contamination
            context = session_data.get('cwe_context', {})
            contamination = self.security_validator.detect_context_contamination(session_id, context)
            assert contamination is False
