...
import pytest
from playwright.async_api import Page, BrowserType, expect
import asyncio
import time
from utils.chainlit_helpers import ChainlitTestHelper, setup_test_environment
from utils.role_helpers import get_test_query_for_role
...
class TestCrossBrowserCompatibility:
    ...
    @pytest.mark.asyncio
    async def test_application_loads_across_browsers(self, playwright, browser_name: str, chainlit_base_url: str):
    ...
    @pytest.mark.asyncio
    async def test_basic_ui_interactions_cross_browser(self, playwright, browser_name: str, chainlit_base_url: str):
    ...
    @pytest.mark.asyncio
    async def test_javascript_compatibility_cross_browser(self, playwright, browser_name: str, chainlit_base_url: str):
        ...
        try:
            ...
            if result.get('jsEnabled'):
                ...
                print(f"  ❌ JavaScript issues in {browser_name}: {result.get('error', 'Unknown')}")
    ...
    @pytest.mark.asyncio
    async def test_websocket_connectivity_cross_browser(self, playwright, browser_name: str, chainlit_base_url: str):
        ...
        try:
            ...
            websocket_connected = any(event['event'] == 'connected' for event in websocket_events)
            
            if websocket_connected:
                print(f"  ✓ WebSocket connected in {browser_name}")
                
                # Check for message exchange
                messages_sent = sum(1 for event in websocket_events if event['event'] == 'frame_sent')
                messages_received = sum(1 for event in websocket_events if event['event'] == 'frame_received')
...
class TestPerformanceAcrossBrowsers:
    ...
    @pytest.mark.parametrize("browser_name", ["chromium", "firefox", "webkit"])
    @pytest.mark.asyncio
    async def test_page_load_performance_cross_browser(self, playwright, browser_name: str, chainlit_base_url: str):
        """Test page load performance across browsers."""
        print(f"\n=== Testing page load performance in {browser_name.upper()} ===")
        
        browser_type = getattr(playwright, browser_name)
        browser = await browser_type.launch(headless=True)  # Headless for more consistent performance
    ...
    @pytest.mark.parametrize("browser_name", ["chromium", "firefox", "webkit"])
    @pytest.mark.asyncio
    async def test_query_response_performance_cross_browser(self, playwright, browser_name: str, chainlit_base_url: str):
    ...
    @pytest.mark.parametrize("browser_name", ["chromium", "firefox", "webkit"])
    @pytest.mark.asyncio
    async def test_memory_usage_cross_browser(self, playwright, browser_name: str, chainlit_base_url: str):
        ...
        try:
            ...
            if initial_metrics:
                print(f"  Initial memory usage: {initial_metrics['usedJSHeapSize'] / 1024 / 1024:.2f} MB")
                print(f"  Total heap size: {initial_metrics['totalJSHeapSize'] / 1024 / 1024:.2f} MB")
            else:
            ...
            for i in range(5):
                await helper.submit_message(f"Test query {i+1}: What is vulnerability assessment?")
            ...
            if initial_metrics and final_metrics:
                ...
                print(f"  Final memory usage: {final_metrics['usedJSHeapSize'] / 1024 / 1024:.2f} MB")
...
class TestBrowserSpecificFeatures:
    ...
    @pytest.mark.parametrize("browser_name", ["chromium", "firefox", "webkit"])
    @pytest.mark.asyncio
    async def test_css_compatibility_cross_browser(self, playwright, browser_name: str, chainlit_base_url: str):
    ...
    @pytest.mark.parametrize("browser_name", ["chromium", "firefox", "webkit"])
    @pytest.mark.asyncio
    async def test_accessibility_features_cross_browser(self, playwright, browser_name: str, chainlit_base_url: str):
        ...
        try:
            ...
            print(f"  Color scheme: {contrast_info['color']} on {contrast_info['backgroundColor']}")
...
class TestMobileCompatibility:
    ...
    @pytest.mark.parametrize("browser_name", ["chromium", "webkit"])  # Firefox mobile support limited
    @pytest.mark.asyncio
    async def test_mobile_viewport_compatibility(self, playwright, browser_name: str, chainlit_base_url: str):
        ...
        try:
            for device in mobile_devices:
                ...
                print(f"  Testing {device['name']} ({device['viewport']['width']}x{device['viewport']['height']})...")
                ...
                print(f"    Actual viewport: {mobile_features['viewportWidth']}x{mobile_features['viewportHeight']}")
                ...
                await take_test_screenshot(page, f"mobile_{browser_name}_{device['name'].replace(' ', '_')}")
...
class TestNetworkConditions:
    ...
    @pytest.mark.parametrize("browser_name", ["chromium"])  # Network throttling best supported in Chromium
    @pytest.mark.asyncio
    async def test_slow_network_performance(self, playwright, browser_name: str, chainlit_base_url: str):
        ...
        try:
            ...
            if response and len(response) > 10:
                print(f"  ✓ Application functional on slow network")
            else:
                print(f"  ⚠ Application may have issues on slow network")
            
            if load_time < 30.0 and response_time < 60.0:
                print(f"  ✓ Acceptable performance on slow network")
            else:
                print(f"  ⚠ Poor performance on slow network")
    ...
    @pytest.mark.asyncio
    async def test_offline_behavior(self, playwright, browser_name: str, chainlit_base_url: str):
        ...
        try:
            ...
            try:
                ...
                if response:
                    print(f"  ⚠ Application provided response while offline: {response[:50]}...")
                else:
            ...
            try:
                ...
                print(f"  ✓ Navigation failed while offline (expected)")
...
class TestCrossBrowserSummary:
    ...
    @pytest.mark.asyncio
    async def test_generate_cross_browser_report(self, playwright, chainlit_base_url: str):
            ...
            try:
                ...
                if results['loads']:
                    try:
                        ...
                        if response and len(response) > 10:
                            ...
                            print(f"  ⚠ Limited interactivity")
                    except Exception as e:
                        results['errors'].append(f"Interactivity failed: {e}")
                        print(f"  ❌ Interactivity failed")
                ...
                if results['interactive']:
                    try:
                        ...
                        print(f"  Performance: {results['performance']} ({load_time:.2f}s)")
                    except Exception as e:
        ...
        # Overall assessment
        working_browsers = sum(1 for results in compatibility_results.values() if results['loads'] and results['interactive'])
