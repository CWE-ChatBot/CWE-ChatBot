...
class ScreenshotHelper:
    ...
    async def capture_page_state(self, page: Page, test_name: str) -> Dict[str, str]:
        ...
        full_page_path = await self.capture_screenshot(page, f"{clean_name}_full", full_page=True)
        if full_page_path:
            screenshots["full_page"] = full_page_path
        
        # Viewport screenshot
        viewport_path = await self.capture_screenshot(page, f"{clean_name}_viewport", full_page=False)
        if viewport_path:
...
async def take_test_screenshot(page: Page, name: str, helper: Optional[ScreenshotHelper] = None) -> str:
...
async def capture_ui_state_for_role(page: Page, role_name: str, helper: Optional[ScreenshotHelper] = None) -> Dict[str, str]:
...
async def capture_progressive_disclosure_state(
    page: Page, 
    action_name: str, 
    helper: Optional[ScreenshotHelper] = None
) -> str:
...
class VisualRegressionTest:
    ...
    async def assert_visual_match(self, page: Page, test_name: str, threshold: float = 0.1) -> bool:
        ...
        if not comparison["match"]:
            print(f"Visual regression test failed: Screenshot does not match baseline")
