...
class RoleTestHelper:
    ...
    ROLE_RESPONSE_INDICATORS = {
        UserRole.PSIRT: [
            "vulnerability", "advisory", "impact", "severity", "CVSS", 
            "affected systems", "mitigation", "disclosure"
        ],
        UserRole.DEVELOPER: [
            "code", "fix", "implement", "prevent", "secure coding", 
            "validation", "sanitization", "best practices"
        ],
        UserRole.ACADEMIC: [
            "research", "study", "analysis", "relationship", "trend", 
            "statistical", "comparative", "taxonomy"
        ],
        UserRole.BUG_BOUNTY: [
            "exploit", "finding", "report", "bounty", "detection", 
            "payload", "reconnaissance", "proof of concept"
        ],
        UserRole.PRODUCT_MANAGER: [
            "business", "impact", "priority", "risk", "cost", 
            "timeline", "resource", "strategy", "roadmap"
        ]
    }
    ...
    ROLE_PROGRESSIVE_ACTIONS = {
        ...
        UserRole.PRODUCT_MANAGER: ["show_business_impact", "show_priority", "tell_more"]
    }
    ...
    @classmethod
    def validate_progressive_actions_present(cls, page_content: str, role: UserRole) -> bool:
        ...
        present_actions = sum(1 for action in expected_actions 
...
CWE_TEST_SCENARIOS = {
    ...
    "sql_injection": {
        ...
        "expected_keywords": ["parameterized queries", "prepared statements", "input validation"]
    },
    "path_traversal": {
        "cwe_id": "CWE-22",
        "query": "What is directory traversal vulnerability?",
        "expected_keywords": ["path traversal", "directory", "file access", "input validation"]
    }
}