...
class CommonUIScenarios:
    ...
    BASIC_NAVIGATION = UIScenario(
        ...
        expected_outcomes=["Application loads successfully", "Main UI elements are visible"]
    )
    ...
    ROLE_SELECTION = UIScenario(
        ...
        expected_outcomes=["Role is selected successfully", "UI adapts to selected role"]
    )
    ...
    QUERY_SUBMISSION = UIScenario(
        ...
        expected_outcomes=["Query is submitted successfully", "Response is received and displayed"]
    )
...
class ScenarioExecutor:
    ...
    async def execute_scenario(self, scenario: UIScenario, context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    async def _execute_actions(self, actions: List[UIAction], context: Dict[str, Any], result: Dict[str, Any]):
        ...
        for action in actions:
            try:
                ...
                if not action.optional:
                    result["success"] = False
                    result["errors"].append(f"Action '{action.description}' failed: {str(e)}")
                    raise
                else:
                    result["errors"].append(f"Optional action '{action.description}' skipped: {str(e)}")
    ...
    async def _execute_action(self, action: UIAction, context: Dict[str, Any]):
        ...
        if action.action_type == InteractionType.NAVIGATION:
            ...
            await self.page.screenshot(path=f"test-results/screenshots/{screenshot_name}.png")
...
async def execute_quick_test(page: Page, scenario_name: str, context: Dict[str, Any] = None) -> bool:
