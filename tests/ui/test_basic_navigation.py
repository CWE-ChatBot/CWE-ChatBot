...
class TestBasicNavigation:
    ...
    @pytest.mark.asyncio
    async def test_application_loads_successfully(self, page: Page, chainlit_base_url: str):
    ...
    @pytest.mark.asyncio
    async def test_chainlit_interface_elements_present(self, page: Page, chainlit_base_url: str):
        ...
        assert element_found, f"No Chainlit interface elements found. Tried selectors: {possible_selectors}"
        
        # Take screenshot of the interface
        await take_test_screenshot(page, "chainlit_interface_loaded")
    
    @pytest.mark.asyncio
    async def test_application_responsive_design(self, page: Page, chainlit_base_url: str):
    ...
    async def test_page_load_performance(self, page: Page, chainlit_base_url: str):
        ...
        assert load_time < 10.0, f"Page load time {load_time:.2f}s exceeds 10 second limit"
    ...
    async def test_no_console_errors_on_load(self, page: Page, chainlit_base_url: str):
        ...
        critical_errors = [error for error in errors if not any(
            acceptable in error.lower() for acceptable in [
                "favicon", "404", "websocket", "development", "warning"
            ]
        )]
    ...
    async def test_using_scenario_executor(self, page: Page, chainlit_base_url: str):
        ...
        result = await executor.execute_scenario(scenario, {"base_url": chainlit_base_url})
        
        # Verify scenario executed successfully
        assert result["success"], f"Navigation scenario failed: {result['errors']}"
        assert result["actions_completed"] > 0, "At least one action should have completed"
...
class TestErrorHandling:
    ...
    async def test_invalid_url_handling(self, page: Page):
        ...
        try:
            ...
            assert response.status in [404, 200], f"Unexpected status code: {response.status}"
    ...
    async def test_application_offline_behavior(self, page: Page, chainlit_base_url: str):
        ...
        try:
            ...
            assert any(keyword in str(e).lower() for keyword in ["timeout", "offline", "connection"]), \
