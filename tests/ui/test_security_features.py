    ...
    async def test_xss_input_sanitization(self, page: Page, chainlit_base_url: str):
        ...
        for i, payload in enumerate(xss_payloads):
            ...
            try:
                ...
                print(
                    f"  Original payload in response: {'⚠ YES' if payload_in_response else '✓ NO'}"
                )
                ...
                print(
                    f"  Sanitization indicators present: {'✓ YES' if sanitized else '⚠ NO'}"
                )
                ...
                print(
                    f"  Injected scripts detected: {'⚠ YES' if injected_scripts > 0 else '✓ NO'}"
                )
                ...
                try:
                    ...
                    print(
                        f"  Alert dialogs triggered: {'⚠ YES' if dialog_appeared else '✓ NO'}"
                    )
                ...
                if not payload_in_response and not dialog_appeared:
                    ...
                    print("  ⚠ XSS payload may not be properly sanitized")

            except Exception as e:
                print(f"  ℹ XSS test {i+1} error: {e}")

    @pytest.mark.asyncio
    async def test_sql_injection_input_sanitization(
        ...
        for i, payload in enumerate(sql_payloads):
            ...
            try:
                ...
                print(
                    f"  SQL keywords in response: {'⚠ YES' if dangerous_sql_present else '✓ SAFE'}"
                )
                ...
                sql_errors = any(
                    indicator in response.lower() for indicator in sql_error_indicators
                )
                ...
                print(
                    f"  Input sanitization applied: {'✓ YES' if sanitized else '⚠ NO'}"
                )
                ...
                if not dangerous_sql_present and not sql_errors:
                    print("  ✓ SQL injection payload properly handled")
                else:
                    print("  ⚠ SQL injection payload may need better handling")
    ...
    async def test_prompt_injection_sanitization(
        ...
        for i, payload in enumerate(prompt_injections):
            ...
            try:
                ...
                print(
                    f"  Injection keywords reflected: {'⚠ YES' if injection_reflected else '✓ NO'}"
                )
                ...
                security_response = any(
                    phrase in response.lower() for phrase in security_responses
                )

                print(
                    f"  Security response provided: {'✓ YES' if security_response else '⚠ NO'}"
                )
                ...
                stays_on_topic = any(
                    keyword in response.lower() for keyword in cwe_keywords
                )

                print(
                    f"  Response stays on topic: {'✓ YES' if stays_on_topic else '⚠ NO'}"
                )
                ...
                if not injection_reflected and (security_response or stays_on_topic):
                    print("  ✓ Prompt injection properly handled")
                else:
                    print("  ⚠ Prompt injection may need better handling")

            except Exception as e:
                print(f"  ℹ Prompt injection test {i+1} error: {e}")

    @pytest.mark.asyncio
    async def test_path_traversal_input_sanitization(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        for i, payload in enumerate(path_payloads):
            ...
            try:
                ...
                system_content_leaked = any(
                    indicator in response for indicator in system_indicators
                )

                print(
                    f"  System content leaked: {'⚠ YES' if system_content_leaked else '✓ NO'}"
                )
                ...
                print(
                    f"  Path traversal reflected: {'⚠ YES' if path_reflected else '✓ SAFE'}"
                )

                # Check for security handling
                security_indicators = ["invalid", "blocked", "not allowed", "sanitized"]
                security_handled = any(
                    indicator in response.lower() for indicator in security_indicators
                )

                print(
                    f"  Security handling applied: {'✓ YES' if security_handled else '⚠ NO'}"
                )

                await take_test_screenshot(page, f"path_traversal_test_{i+1}")

                if not system_content_leaked and not path_reflected:
                    print("  ✓ Path traversal payload properly sanitized")
                else:
                    print("  ⚠ Path traversal payload needs better sanitization")
    ...
    async def test_session_storage_encryption(self, page: Page, chainlit_base_url: str):
        ...
        try:
            ...
            role_related_keys = [
                ...
                if any(
                    term in key.lower() for term in ["role", "user", "session", "auth"]
                )
            ]
            ...
            if role_related_keys:
                ...
                for key in role_related_keys:
                    ...
                    contains_plain_role = any(
                        role in value.lower() for role in plain_text_roles
                    )

                    print(
                        f"    Plain text role detected: {'⚠ YES' if contains_plain_role else '✓ NO'}"
                    )

                    # Check for encryption characteristics
                    looks_encrypted = len(value) > 20 and not any(
                        c.isalpha() and c.islower() for c in value[:10]
                    )

                    print(
                        f"    Appears encrypted: {'✓ YES' if looks_encrypted else '⚠ NO'}"
                    )
    ...
    async def test_session_data_isolation(self, page: Page, chainlit_base_url: str):
        ...
        try:
            ...
            if session_data1 and session_data2:
                ...
                print(
                    f"  Sessions are isolated: {'✓ YES' if sessions_different else '⚠ NO'}"
                )
            else:
    ...
    async def test_session_timeout_behavior(self, page: Page, chainlit_base_url: str):
        ...
        try:
            ...
            if response:
                ...
                handles_gracefully = not any(
                    error in response.lower()
                    for error in ["error", "exception", "undefined", "null"]
                )
                print(
                    f"  Graceful session handling: {'✓ YES' if handles_gracefully else '⚠ NO'}"
                )
        ...
        try:
            ...
            for i, request in enumerate(requests_made):
                ...
                if request["post_data"]:
                    if (
                        "csrf" in request["post_data"].lower()
                        or "token" in request["post_data"].lower()
                    ):
...
class TestSecurityEventLogging:
    ...
    @pytest.mark.asyncio
    async def test_security_event_console_logging(
        self, page: Page, chainlit_base_url: str
    ):
    ...
    async def test_security_monitoring_integration(
        ...
        def handle_request(request):
            if any(
                ...
                for keyword in ["security", "audit", "log", "monitor", "alert"]
            ):
...
class TestSecurityPerformance:
    ...
    @pytest.mark.asyncio
    async def test_security_feature_performance_impact(
        self, page: Page, chainlit_base_url: str
    ):
    ...
    async def test_complete_security_workflow(self, page: Page, chainlit_base_url: str):
        ...
        try:
            ...
            await helper.submit_message(
                "<script>alert('test')</script>Tell me about XSS"
            )
            response = await helper.get_last_response()
            ...
            encrypted_session = len(session_data) == 0 or not any(
                "developer" in str(data).lower() for data in session_data
            )
            ...
            print(
                f"  Overall security: {'✓ GOOD' if security_features_working else '⚠ NEEDS ATTENTION'}"
            )
...
class TestAdvancedSecurityFeatures:
    ...
    @pytest.mark.asyncio
    async def test_content_security_policy_enforcement(
        self, page: Page, chainlit_base_url: str
    ):
