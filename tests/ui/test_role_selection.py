...
from utils.role_helpers import (
    RoleTestHelper,
    get_all_test_roles,
    get_test_query_for_role,
    validate_role_response_format,
)
...
class TestRoleSelection:
    ...
    @pytest.mark.asyncio
    async def test_role_selection_interface_present(
    ...
    @pytest.mark.asyncio
    async def test_role_selection_for_each_role(
        ...
        try:
            ...
            await capture_ui_state_for_role(
                page, f"role_selection_pending_{role.value}"
            )
...
class TestRoleBasedResponses:
    ...
    @pytest.mark.asyncio
    async def test_role_specific_response_format(
        self, page: Page, chainlit_base_url: str, role: UserRole
    ):
        ...
        try:
            ...
            for i, query in enumerate(
                ...
                if response and len(response) > 10:
                    ...
                    print(
                        f"  Format validation: {'✓ PASSED' if is_valid_format else '⚠ NEEDS REVIEW'}"
                    )

                    if not is_valid_format:
                        # Log details for debugging
                        expected_indicators = (
                            RoleTestHelper.ROLE_RESPONSE_INDICATORS.get(role, [])
                        )
                    ...
                    await capture_ui_state_for_role(
                        page, f"response_{role.value}_query_{i+1}"
                    )
                else:
    ...
    @pytest.mark.asyncio
    async def test_role_switching_response_differences(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        for role in test_roles:
            try:
                ...
                if response and len(response) > 10:
                    ...
                    print(
                        f"  ✓ Response captured for {role.value}: {len(response)} characters"
                    )

                    # Take screenshot
                    await capture_ui_state_for_role(
                        page, f"role_comparison_{role.value}"
                    )
                else:
        ...
        if len(role_responses) >= 2:
            ...
            for i, role1 in enumerate(roles):
                for role2 in roles[i + 1 :]:
                    ...
                    similarity = len(
                        set(response1.lower().split()) & set(response2.lower().split())
                    )
                    total_words = len(
                        set(response1.lower().split()) | set(response2.lower().split())
                    )
                    similarity_ratio = (
                        similarity / total_words if total_words > 0 else 0
                    )

                    print(
                        f"  {role1} vs {role2}: {similarity_ratio:.2f} word overlap ratio"
                    )

                    if similarity_ratio < 0.8:  # Responses are sufficiently different
                        print("    ✓ Responses appropriately differentiated")
                    else:
                        print("    ⚠ Responses may be too similar")
        else:
...
class TestRoleContextPreservation:
    ...
    @pytest.mark.asyncio
    async def test_role_context_across_multiple_queries(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        try:
            ...
            for i, query in enumerate(test_queries):
                ...
                if response and len(response) > 10:
                    ...
                    is_developer_focused = validate_role_response_format(
                        response, test_role
                    )
                    print(
                        f"  Developer focus maintained: {'✓' if is_developer_focused else '⚠'}"
                    )

                    # Take screenshot
                    await capture_ui_state_for_role(
                        page, f"context_preservation_query_{i+1}"
                    )
                else:
            ...
            if len(responses) >= 2:
                ...
                all_role_appropriate = all(
                    validate_role_response_format(resp, test_role) for resp in responses
                )
    ...
    @pytest.mark.asyncio
    async def test_role_context_after_page_refresh(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        try:
            ...
            if post_refresh_response and len(post_refresh_response) > 10:
                print(
                    f"✓ Post-refresh response received: {len(post_refresh_response)} characters"
                )

                # Check if role context was maintained
                is_role_maintained = validate_role_response_format(
                    post_refresh_response, test_role
                )
                print(
                    f"Role context after refresh: {'✓ MAINTAINED' if is_role_maintained else '⚠ LOST'}"
                )
    ...
    async def test_role_switching_workflow(self, page: Page, chainlit_base_url: str):
        ...
        try:
            ...
            if response1 and response2:
                ...
                print(
                    f"Responses differ after role switch: {'✓' if responses_different else '⚠'}"
                )
                ...
                if responses_different:
                    ...
                    print(
                        f"{role1.value} response appropriate: {'✓' if role1_appropriate else '⚠'}"
                    )
                    print(
                        f"{role2.value} response appropriate: {'✓' if role2_appropriate else '⚠'}"
                    )
                    ...
                    print(
                        "ℹ Responses identical - role switching may not be implemented"
                    )
            else:
