...
from utils.role_helpers import RoleTestHelper
...
class TestProgressiveDisclosureButtons:
    ...
    @pytest.mark.asyncio
    async def test_progressive_disclosure_buttons_present(
        ...
        try:
            ...
            if response and len(response) > 10:
                ...
                if buttons_found:
                    print(
                        f"✓ Progressive disclosure buttons found: {len(buttons_found)}"
                    )
                ...
                await capture_progressive_disclosure_state(
                    page, "buttons_initial_state"
                )
            else:
    ...
    @pytest.mark.asyncio
    async def test_progressive_action_buttons(
        self, page: Page, chainlit_base_url: str, action_type: str
    ):
            ...
            for selector in button_selectors:
                try:
                    ...
                    if await button.is_visible() and await button.is_enabled():
                        ...
                        await capture_progressive_disclosure_state(
                            page, f"{action_type}_before_click"
                        )
                        ...
                        await capture_progressive_disclosure_state(
                            page, f"{action_type}_after_click"
                        )
                        ...
                        if post_click_response != initial_response:
                            ...
                            print(
                                f"  New content length: {len(post_click_response)} characters"
                            )
                        else:
                        ...
                        print(
                            f"  Button disabled after click: {'✓' if is_disabled else '⚠'}"
                        )
    ...
    @pytest.mark.asyncio
    async def test_multiple_progressive_actions_sequence(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        try:
            ...
            for i, action in enumerate(action_sequence):
                ...
                try:
                    ...
                    # Take screenshot
                    await capture_progressive_disclosure_state(
                        page, f"sequence_step_{i+1}_{action}"
                    )
...
class TestProgressiveDisclosureRoleIntegration:
    ...
    @pytest.mark.parametrize(
        "role", [UserRole.PSIRT, UserRole.DEVELOPER, UserRole.ACADEMIC]
    )
    @pytest.mark.asyncio
    async def test_role_specific_progressive_actions(
        self, page: Page, chainlit_base_url: str, role: UserRole
    ):
        ...
        try:
            ...
            for action in expected_actions:
                try:
                    ...
                    # Take screenshot
                    await capture_progressive_disclosure_state(
                        page, f"role_{role.value}_{action}"
                    )
    ...
    @pytest.mark.asyncio
    async def test_progressive_actions_adapt_to_role_switch(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        try:
            print(
                f"Testing progressive action adaptation: {role1.value} → {role2.value}"
            )
            ...
            role1_actions = []
            expected_role1_actions = RoleTestHelper.get_expected_progressive_actions(
                role1
            )
            ...
            await capture_progressive_disclosure_state(
                page, f"role_adaptation_{role1.value}"
            )
            ...
            role2_actions = []
            expected_role2_actions = RoleTestHelper.get_expected_progressive_actions(
                role2
            )
            ...
            await capture_progressive_disclosure_state(
                page, f"role_adaptation_{role2.value}"
            )
...
class TestProgressiveDisclosureState:
    ...
    async def test_button_state_management(self, page: Page, chainlit_base_url: str):
        ...
        try:
            ...
            for i, button_info in enumerate(test_buttons[:2]):  # Test first 2 buttons
                ...
                print(
                    f"    Initial state: {'enabled' if initial_enabled else 'disabled'}"
                )
                ...
                if initial_enabled:
                    ...
                    print(
                        f"    Post-click state: {'enabled' if post_click_enabled else 'disabled'}"
                    )
                    ...
                    await capture_progressive_disclosure_state(
                        page, f"button_state_{i+1}"
                    )

                    if not post_click_enabled and content_changed:
                        print("    ✓ Button correctly disabled after successful action")
                    elif post_click_enabled and not content_changed:
                        print("    ⚠ Button still enabled but no content change")
                    elif not post_click_enabled and not content_changed:
                        print("    ⚠ Button disabled but no content loaded")
                else:
                    print("    ℹ Button initially disabled")

        except Exception as e:
            print(f"ℹ Button state management testing: {e}")

    @pytest.mark.asyncio
    async def test_progressive_content_loading_indicators(
        self, page: Page, chainlit_base_url: str
    ):
    ...
    @pytest.mark.asyncio
    async def test_progressive_content_organization(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        try:
            ...
            await capture_progressive_disclosure_state(
                page, "content_organization_initial"
            )
                ...
                try:
                    ...
                    if len(content_sections) > 1:
                        ...
                        if len(current_response) > len(previous_content):
                            print(
                                f"    ✓ Content expanded (was {len(previous_content)}, now {len(current_response)} chars)"
                            )
                        else:
                            print(
                                "    ⚠ Content may have been replaced rather than expanded"
                            )
            ...
            if content_sections:
                ...
                initial_content = (
                    content_sections[0] if content_sections else initial_response
                )
                ...
                print(
                    f"    Expansion ratio: {len(final_content) / len(initial_content):.1f}x"
                )
...
class TestProgressiveDisclosurePerformance:
    ...
    @pytest.mark.asyncio
    async def test_progressive_action_response_time(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        test_query = (
            "What are the security implications of XML External Entity attacks?"
        )
        ...
        try:
            ...
            for action in actions_to_test:
                try:
                    ...
                    performance_results.append(
                        {"action": action, "response_time": response_time}
                    )
            ...
            if performance_results:
                ...
                avg_time = (
                    sum(r["response_time"] for r in performance_results) / total_tests
                )
                ...
                fast_responses = sum(
                    1 for r in performance_results if r["response_time"] < 3.0
                )
                acceptable_responses = sum(
                    1 for r in performance_results if r["response_time"] < 10.0
                )

                print(f"Fast responses (<3s): {fast_responses}/{total_tests}")
                print(
                    f"Acceptable responses (<10s): {acceptable_responses}/{total_tests}"
                )
...
class TestRoleProgressiveIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_role_progressive_workflow(
        self, page: Page, chainlit_base_url: str
    ):
        ...
        try:
            ...
            expected_actions = RoleTestHelper.get_expected_progressive_actions(
                test_role
            )
            ...
            if successful_actions > 0:
                ...
                print(
                    f"    ℹ Progressive disclosure not yet functional for {test_role.value}"
                )

            # Take final screenshot
            await capture_progressive_disclosure_state(
                page, f"complete_workflow_{test_role.value}"
            )
...
class TestProgressiveImplementationReady:
    ...
    @pytest.mark.asyncio
    async def test_all_progressive_actions_functional(
        self, page: Page, chainlit_base_url: str
    ):
    ...
    @pytest.mark.asyncio
    async def test_progressive_csrf_protection(
        self, page: Page, chainlit_base_url: str
    ):
