...
**Critical Fix:** Regional endpoint required (`us-central1-modelarmor.googleapis.com`), project-level IAM (`roles/modelarmor.user`)
...
* [x] **T1: Wire Model Armor on serve path** (AC: 1, 2) - ✅ COMPLETE (Model-Agnostic Sanitize APIs)
  ...
  * **PDF Upload Sanitization (PENDING):**
    - PDF text extraction → SanitizeUserPrompt BEFORE sending to LLM
    - Prevents malicious content injection via PDF uploads
    - Integration point: PDF worker service (apps/pdf_worker/)
    - Requires: Model Armor client in PDF worker, same template/endpoint configuration
...
**Layer 1: Model Armor Sanitization** - ✅ DEPLOYED (Revision cwe-chatbot-00154-zl6)
- Implementation: [apps/chatbot/src/model_armor_guard.py](../../apps/chatbot/src/model_armor_guard.py)
- Template: `projects/cwechatbot/locations/us-central1/templates/llm-guardrails-default`
- Endpoint: `us-central1-modelarmor.googleapis.com` (regional endpoint required)
...
- Fail-closed: Generic errors on BLOCK/SANITIZE/INCONCLUSIVE decisions
