...
### Per-User Rate Limits (S-1.1 Enhancement)

**Context**: Story S-1.1 adds per-user rate limiting via Cloud Armor, keyed by trusted `X-User-Id` header.

#### Header Integrity
- **Client-supplied X-User-Id is STRIPPED** at Load Balancer (requestHeadersToRemove)
- **Only trusted gateway** (IAP/API Gateway/JWT middleware) adds X-User-Id **after authentication**
- External clients **cannot spoof** user identity

#### Configuration (Per-User)
- **Limit**: 60 requests/minute per user
- **Window**: 60 seconds
- **Ban duration**: 300 seconds (5 minutes)
- **Priority**: 1000 (evaluated before per-IP rule at 1100)

#### Tuning Knobs
- `RPM_PER_USER`: Requests per minute per user (default: 60)
- `BAN_DURATION_SEC`: Ban duration in seconds (default: 300)
- `BAN_COUNT_MULTIPLIER`: Multiplier for ban threshold (default: 2x)

Keep per-IP rule as **fallback** for:
- Requests without X-User-Id header
- Abusive reconnect loops (WebSocket)
- Defense-in-depth protection

#### Triage Flow: User Throttled
If a specific user is being rate limited:

1. **Verify header integrity**:
   - Check gateway logs to confirm X-User-Id was set by gateway (not client)
   - Look for `enforcedOnKey=HTTP_HEADER` in Cloud Armor logs

2. **Assess legitimacy**:
   - Is this a legitimate user with high usage?
   - Or potential abuse (automated scraping, bot activity)?

3. **Resolution options**:
   - **Temporary increase**: Raise `RPM_PER_USER` for all users (emergency only)
   - **Gateway exception**: Grant specific user higher limit at gateway level
   - **User communication**: Contact user about rate limit policy

4. **Monitor impact**:
   - Watch `rate_limit_blocks` metric for spike
   - Check application latency and error rates
   - Verify ban duration is appropriate (not too long/short)
...
jsonPayload.enforcedAction="DENY_429"
```
...
resource.type="http_load_balancer"
jsonPayload.enforcedAction="DENY_429"
jsonPayload.enforcedOnKey="HTTP_HEADER"
```

**Per-IP fallback blocks**:
```
resource.type="http_load_balancer"
jsonPayload.enforcedAction="DENY_429"
jsonPayload.enforcedOnKey="IP"
```

#### Rule Priority Order
```
1000: Per-user (X-User-Id) → 60 rpm/user, 300s ban
1100: Per-IP → 60 rpm/IP, 300s ban (fallback)
2147483647: Default allow (all other traffic)
```
...
```bash
# Remove per-user rule (keeps per-IP fallback)
gcloud compute security-policies rules delete 1000 \
  --security-policy=cwe-chatbot-armor

# Per-IP rule @ 1100 continues protecting
# No service outage required
```

**Implementation Scripts**:
- `scripts/rollout_per_user_rate_limit.sh` - Idempotent deployment
- `tests/integration/test_rate_limit_user.py` - Integration tests

**See**: [Story S-1.1](S-1.1AddPer-UserEdgeRateLimitsGCPCloudArmor.md) for complete implementation guide
