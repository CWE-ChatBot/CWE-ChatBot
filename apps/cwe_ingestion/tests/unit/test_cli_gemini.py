...
def test_cli_supports_gemini_embedder_option():
    ...
    with patch.dict(
        os.environ, {"GEMINI_API_KEY": "test-key", "DATABASE_URL": "postgresql://test"}
    ):
        ...
        with patch("cli.CWEIngestionPipeline") as mock_pipeline_class:
            ...
            result = runner.invoke(
                cli, ["ingest", "--embedder-type", "gemini", "--target-cwes", "CWE-79"]
            )
...
def test_cli_defaults_to_local_embedder():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": "postgresql://test"}):
        ...
        with patch("cli.CWEIngestionPipeline") as mock_pipeline_class:
            ...
            result = runner.invoke(cli, ["ingest", "--target-cwes", "CWE-79"])
...
def test_cli_validates_embedder_type():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": "postgresql://test"}):
        result = runner.invoke(
            cli,
            ["ingest", "--embedder-type", "invalid_type", "--target-cwes", "CWE-79"],
        )
