...
def test_database_features(
    store: PostgresChunkStore, db_name: str, embedder: GeminiEmbedder
):
    ...
    with store.conn.cursor() as cur:
        ...
        if vector_indexes:
            for idx in vector_indexes:
                index_type = (
                    "HNSW"
                    if "hnsw" in idx[3].lower()
                    else "IVFFlat"
                    if "ivfflat" in idx[3].lower()
                    else "Unknown"
                )
    ...
    for i, (test_name, query) in enumerate(test_queries, 1):
        ...
        try:
            ...
            print(
                f"  Sections:  {', '.join(sections_found[:3])}{'...' if len(sections_found) > 3 else ''}"
            )

            if results:
                print(
                    f"  Top CWE:   {results[0]['cwe_id']} - {results[0]['name'][:50]}"
                )
    ...
    for section, query in section_tests:
        try:
            ...
            section_accuracy = (
                len([r for r in results if r["section"] == section]) / len(results)
                if results
                else 0
            )

            print(
                f"  {section:15s}: {query_time*1000:5.1f}ms | {len(results)} results | {section_accuracy*100:4.1f}% accuracy"
            )
    ...
    if successful_queries > 0:
        ...
        print(
            f"  Database status: {'✅ EXCELLENT' if avg_time < 0.1 else '✅ GOOD' if avg_time < 0.5 else '⚠️ SLOW'}"
        )
...
def main():
    ...
    try:
        ...
        print("-" * 50)
        print(
            f"{'Total Chunks':<20} {local_results['total_chunks']:<15,} {prod_results['total_chunks']:<15,}"
        )
        print(
            f"{'Unique CWEs':<20} {local_results['unique_cwes']:<15} {prod_results['unique_cwes']:<15}"
        )
        print(
            f"{'Unique Sections':<20} {local_results['unique_sections']:<15} {prod_results['unique_sections']:<15}"
        )
        print(
            f"{'Vector Indexes':<20} {local_results['vector_indexes']:<15} {prod_results['vector_indexes']:<15}"
        )
        print(
            f"{'Avg Query Time':<20} {local_results['avg_query_time_ms']:<15.1f}ms {prod_results['avg_query_time_ms']:<15.1f}ms"
        )
        print(
            f"{'Success Rate':<20} {local_results['successful_queries']}/10{'':<10} {prod_results['successful_queries']}/10"
        )

        # Overall assessment
        local_ok = (
            local_results["unique_cwes"] == 969
            and local_results["avg_query_time_ms"] < 500
        )
        prod_ok = (
            prod_results["unique_cwes"] == 969
            and prod_results["avg_query_time_ms"] < 500
        )

        print("\n🎯 OVERALL ASSESSMENT:")
        print(
            f"  Local Database:     {'✅ FULLY OPERATIONAL' if local_ok else '❌ ISSUES DETECTED'}"
        )
        print(
            f"  Production Database: {'✅ FULLY OPERATIONAL' if prod_ok else '❌ ISSUES DETECTED'}"
        )
        ...
        if local_ok and prod_ok:
            ...
            print("   - Complete CWE corpus (969 CWEs)")
            print("   - Enhanced chunking (14 sections)")
            print("   - Fast retrieval (sub-500ms)")
