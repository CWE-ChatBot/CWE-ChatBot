...
def test_database_performance(
    ...
    results["original"] = benchmark_query_method(
        store, embedder, "Original vector(3072)", store.query_similar, iterations=5
    )
    ...
    results["halfvec"] = benchmark_query_method(
        store,
        embedder,
        "Optimized halfvec_fast",
        store.query_similar_fast,
        iterations=10,
    )
    ...
    results["hybrid"] = benchmark_query_method(
        store, embedder, "Hybrid halfvec_fast", store.query_hybrid_fast, iterations=10
    )
...
def main():
    ...
    try:
            ...
            for method in ["original", "halfvec", "hybrid"]:
                if method in local_results and method in prod_results:
                    ...
                    if method != "original" and "original" in local_results:
                        ...
                        speedup = (
                            f"{original_p95/local_p95:.1f}x" if local_p95 > 0 else "N/A"
                        )
                    else:
                        speedup = "baseline"

                    print(
                        f"{method:<20} {local_p95:<12.1f} {prod_p95:<12.1f} {speedup}"
                    )
        ...
        if local_results and "halfvec" in local_results:
            ...
            print(
                f"  Local halfvec p95: {local_p95:.1f}ms ({'✅ TARGET MET' if target_met else '⚠️ ABOVE TARGET'})"
            )
        ...
        if prod_results and "halfvec" in prod_results:
            ...
            print(
                f"  Prod halfvec p95: {prod_p95:.1f}ms ({'✅ ACCEPTABLE' if prod_target_met else '⚠️ SLOW'})"
            )

        print("\n📊 Next steps:")
        print("  - Use query_similar_fast() for pure vector search")
        print("  - Use query_hybrid_fast() for keyword+semantic search")
        print("  - Deploy app in us-central1 for production optimization")
