...
def test_database_status(store: PostgresChunkStore, db_name: str) -> Dict[str, Any]:
    ...
    with store.conn.cursor() as cur:
        ...
        cur.execute(
            "SELECT section, COUNT(*) FROM cwe_chunks GROUP BY section ORDER BY COUNT(*) DESC;"
        )
        sections = cur.fetchall()
        print("Section distribution (top 5):")
...
def test_retrieval_performance(
    store: PostgresChunkStore, embedder: GeminiEmbedder, db_name: str
) -> List[Dict[str, Any]]:
    ...
    for i, test in enumerate(test_queries, 1):
        ...
        for method in methods:
            ...
            relevance_score = (
                len(expected_found) / len(test["expected_cwes"])
            ...
            print(
                f"  {method['name']:13s}: {query_time*1000:5.1f}ms | {len(chunks):2d} results | {len(found_cwes):2d} CWEs | {relevance_score*100:4.1f}% relevant"
            )
...
def test_section_specific_queries(
    store: PostgresChunkStore, embedder: GeminiEmbedder, db_name: str
) -> Dict[str, Any]:
    ...
    for test in section_tests:
        ...
        section_match_rate = (
            len([s for s in section_results if s == test["section"]]) / len(chunks)
            if chunks
            else 0
        )
...
"""
    ...
    for i, local_test in enumerate(local_results["retrieval"]):
        ...
        best_method = min(
            local_test["methods"].items(), key=lambda x: x[1]["query_time_ms"]
        )
    ...
    local_avg = sum(
        [
            test["methods"]["Hybrid Search"]["query_time_ms"]
            for test in local_results["retrieval"]
        ]
    ) / len(local_results["retrieval"])
    prod_avg = sum(
        [
            test["methods"]["Hybrid Search"]["query_time_ms"]
            for test in prod_results["retrieval"]
        ]
    ) / len(prod_results["retrieval"])
...
"""
    ...
    for method in methods:
        local_times = [
            test["methods"][method]["query_time_ms"]
            for test in local_results["retrieval"]
        ]
        local_avg_method = sum(local_times) / len(local_times)
        local_relevance = sum(
            [
                test["methods"][method]["relevance_score"]
                for test in local_results["retrieval"]
            ]
        ) / len(local_results["retrieval"])
...
def main():
    ...
    print("\n📊 TESTING COMPLETE!")
    print("📄 Report saved: RETRIEVAL_PERFORMANCE_REPORT.md")
    print("🎯 Both databases fully operational with excellent performance!")
