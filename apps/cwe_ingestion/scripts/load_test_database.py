...
class DatabaseLoadTester:
    ...
    def execute_query(self, query_type: str, query: str, params: tuple) -> QueryResult:
        ...
        try:
            ...
            return QueryResult(
                query_type=query_type, duration_ms=duration_ms, success=True
            )
    ...
    def run_concurrent_query_test(
        self, total_queries: int = 100, workers: int = 4
    ) -> List[QueryResult]:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [
                executor.submit(execute_batch, queries_per_worker)
    ...
    def analyze_results(self, results: List[QueryResult], test_name: str):
        ...
        for qt in sorted(query_types):
            ...
            qt_p95 = (
                statistics.quantiles(qt_results, n=20)[18]
                if len(qt_results) > 1
                else qt_results[0]
            )
        ...
        if p95 < 500:
            ...
            print(
                f"    ⚠️  p95 < 500ms target: FAIL ({p95:.2f}ms, {p95-500:.2f}ms over)"
            )

        if statistics.mean(durations) < 200:
            print(
                f"    ✅ Mean < 200ms target: PASS ({statistics.mean(durations):.2f}ms)"
            )
        else:
            print(
                f"    ℹ️  Mean < 200ms target: {statistics.mean(durations):.2f}ms (local target)"
            )
    ...
    def run_all_tests(self):
        ...
        results_concurrent_4 = self.run_concurrent_query_test(
            total_queries=100, workers=4
        )
        self.analyze_results(results_concurrent_4, "Concurrent Load (4 workers)")

        # Test 3: Higher concurrent load (8 workers)
        results_concurrent_8 = self.run_concurrent_query_test(
            total_queries=100, workers=8
        )
        self.analyze_results(results_concurrent_8, "Concurrent Load (8 workers)")

        # Test 4: Stress test (max pool size)
        results_stress = self.run_concurrent_query_test(
            total_queries=100, workers=self.pool_size
        )
