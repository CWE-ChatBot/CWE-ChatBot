...
def test_database_status(store: PostgresChunkStore, db_name: str):
    ...
    with store.conn.cursor() as cur:
        ...
        cur.execute(
            "SELECT section, COUNT(*) FROM cwe_chunks GROUP BY section ORDER BY COUNT(*) DESC LIMIT 5;"
        )
...
def test_vector_search_only(
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        try:
            ...
            print(
                f"  Sections:  {', '.join(sections[:3])}{'...' if len(sections) > 3 else ''}"
            )

            if chunks:
                top_chunk = chunks[0]
                print(
                    f"  Top result: {top_chunk['metadata']['cwe_id']} - {top_chunk['metadata']['name'][:40]}..."
                )
    ...
    if successful_queries > 0:
        ...
        print(
            f"  Performance: {'✅ EXCELLENT' if avg_time < 0.1 else '✅ GOOD' if avg_time < 0.5 else '⚠️ SLOW'}"
        )
...
def main():
    ...
    try:
        ...
        print("\n" + "=" * 80)
        prod_vector_results = test_vector_search_only(
            prod_store, embedder, "production"
        )
        ...
        local_good = (
            local_status["unique_cwes"] >= 960
            and len(local_vector_results) > 0
            and len(local_text_results) > 0
        )

        prod_good = (
            prod_status["unique_cwes"] >= 960
            and len(prod_vector_results) > 0
            and len(prod_text_results) > 0
        )

        print(
            f"Local Database:      {'✅ FULLY OPERATIONAL' if local_good else '❌ ISSUES DETECTED'}"
        )
        print(
            f"Production Database: {'✅ FULLY OPERATIONAL' if prod_good else '❌ ISSUES DETECTED'}"
        )
        ...
        if local_good and prod_good:
            ...
            print(
                f"   - Vector search working (pgvector {local_status.get('vector_version', 'detected')})"
            )
            print(f"   - Text search working (PostgreSQL {local_status['pg_version']})")
            print("   - Enhanced chunking (multiple sections per CWE)")
        else:
