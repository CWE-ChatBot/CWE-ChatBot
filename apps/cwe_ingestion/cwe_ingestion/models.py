...
def _split_text_into_chunks(
    text: str, target_tokens: int = 500, max_tokens: int = 700, overlap: int = 50
) -> List[str]:
    """
...
def entry_to_sections(entry: "CWEEntry") -> List[SectionDict]:
    ...
    if entry.Description:
        sections.append(
            {"section": "Abstract", "section_rank": rank, "text": entry.Description}
        )
        rank += 1
    ...
    if entry.ExtendedDescription:
        ...
        for ch in ext_chunks:
            sections.append({"section": "Extended", "section_rank": rank, "text": ch})
            rank += 1
        ...
        for phase, items in by_phase.items():
            text = (
                ...
                + "\n".join(
                    ...
                    for mi in items
                    if (mi.Description or mi.Strategy)
                )
            )
            for ch in _split_text_chunks_adaptive(
                ...
                sections.append(
                    {"section": "Mitigations", "section_rank": rank, "text": ch}
                )
                rank += 1
    ...
    if entry.ObservedExamples:
        example_texts = [
            f"- {ex.Reference}: {ex.Description}" for ex in entry.ObservedExamples
        ]
        examples_blob = "Real-World Examples (CVEs):\n" + "\n".join(example_texts)
        for ch in _split_text_chunks_adaptive(
            examples_blob, target_tokens=450, max_tokens=700, overlap=25
        ):
            sections.append({"section": "Examples", "section_rank": rank, "text": ch})
            rank += 1
...
class CWEEntry(BaseModel):
    ...
    def to_searchable_text(self) -> str:
        ...
        if self.RelatedWeaknesses:
            weakness_texts = [
                f"- {w.Nature} of CWE-{w.CweID}" for w in self.RelatedWeaknesses
            ]
        ...
        if self.AlternateTerms:
            term_texts = [
                f"- {term.Term}" + (f": {term.Description}" if term.Description else "")
