...
class FollowupProcessor:
    ...
    def detect_followup_intent(self, query: str) -> FollowupIntent:
        ...
        logger.debug(
            f"Follow-up detection: {query[:50]}... -> {best_intent} (confidence: {best_confidence})"
        )
    ...
    def process_followup_query(
        self, query: str, context_cwe: str, intent: FollowupIntent
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            retrieval_params = self._get_retrieval_params(
                intent.intent_type, context_cwe
            )
            ...
            logger.info(
                f"Processed follow-up query: {intent.intent_type} for {context_cwe}"
            )
    ...
    def _enhance_query_with_context(
        ...
        if intent.intent_type == "tell_more":
            ...
            enhanced_parts.extend(
                ["parent CWEs", "broader categories", "generalizations"]
            )

        elif intent.intent_type == "related":
            enhanced_parts.extend(
                ["related CWEs", "similar vulnerabilities", "connected"]
            )
            ...
            enhanced_parts.extend(
                ["prevention", "mitigation", "countermeasures", "fix"]
            )
    ...
    def _get_retrieval_params(
        self, intent_type: str, context_cwe: str
    ) -> Dict[str, Any]:
        """Get retrieval parameters specific to intent type."""

        params = {"context_cwe": context_cwe, "include_relationships": True, "k": 5}

        # Intent-specific parameters
        if intent_type == "children":
            params.update(
                {"relationship_type": "ChildOf", "relationship_direction": "children"}
            )

        elif intent_type == "parents":
            params.update(
                {"relationship_type": "ParentOf", "relationship_direction": "parents"}
            )

        elif intent_type == "related":
            params.update(
                {"similarity_search": True, "k": 10}  # Get more related items
            )

        elif intent_type in ["tell_more", "consequences", "examples"]:
            params.update(
                {"comprehensive_data": True, "k": 1}  # Just need the specific CWE
            )
