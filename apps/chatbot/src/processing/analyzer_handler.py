...
class AnalyzerModeHandler:
    ...
    async def process(self, query: str, context) -> PipelineResult:
        ...
        try:
            ...
            logger.info(f"Processing analyzer request: '{query[:50]}...' in mode: {current_mode}")
            logger.info(f"Context has last_chunks: {len(getattr(context, 'last_chunks', []))}, last_recommendations: {len(getattr(context, 'last_recommendations', []))}")
    ...
    async def _handle_question_mode(self, query: str, context) -> PipelineResult:
        ...
        logger.info(f"Performing semantic retrieval for follow-up question")
        ...
        if len(chunks_to_use) > max_total_chunks:
            ...
            logger.info(f"Applied circular buffer (limit={max_total_chunks}): reduced from {len(new_chunks) + len(prev_chunks)} to {len(chunks_to_use)} chunks")

        logger.info(f"Combined {len(new_chunks)} new chunks with {len(prev_chunks)} previous chunks (total: {len(chunks_to_use)})")
        ...
        logger.info(f"Using {len(chunks_to_use)} chunks (semantic retrieval + stored context) for follow-up question")
        ...
        try:
            ...
            all_retrieved_cwes = prev_ids + mentioned_cwe_ids if mentioned_cwe_ids else prev_ids
        ...
        context.last_chunks = chunks_to_use  # Store the combined chunks for next question
        logger.info(f"Updated stored context with {len(chunks_to_use)} chunks for future questions")
    ...
    async def _handle_initial_analysis(self, query: str, context) -> PipelineResult:
        ...
        user_prefs = context.get_persona_preferences()
        raw_chunks = await self.pipeline.query_handler.process_query(enhanced_query, user_prefs)
        ...
        context.last_recommendations = result.recommendations if hasattr(result, 'recommendations') else []
        context.last_chunks = raw_chunks  # Store the actual chunks for follow-up

        logger.info(f"Stored analysis results for follow-up: {len(context.last_recommendations)} recommendations, {len(context.last_chunks)} chunks")
