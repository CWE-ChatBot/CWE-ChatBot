...
class TestCWEExtractor:
    ...
    def test_direct_cwe_reference_detection(self):
        ...
        for text in positive_cases:
            assert self.extractor.has_direct_cwe_reference(
                text
            ), f"Should detect CWE in: {text}"

        for text in negative_cases:
            assert not self.extractor.has_direct_cwe_reference(
                text
    ...
    def test_vulnerability_type_keyphrase_extraction(self):
        ...
        for text, expected_phrases in test_cases:
            ...
            for phrase in expected_phrases:
                assert any(
                    phrase in vuln_type.lower() for vuln_type in vuln_types
    ...
    def test_security_terms_extraction(self):
        ...
        for text, expected_terms in test_cases:
            ...
            for term in expected_terms:
                assert any(
                    term in sec_term.lower() for sec_term in security_terms
    ...
    def test_programming_context_extraction(self):
        ...
        for text, expected_contexts in test_cases:
            ...
            for context in expected_contexts:
                assert any(
                    context in prog_context.lower() for prog_context in prog_contexts
    ...
    def test_enhance_query_for_search(self):
        ...
        assert any(
            "xss" in phrase.lower() or "cross-site scripting" in phrase.lower()
