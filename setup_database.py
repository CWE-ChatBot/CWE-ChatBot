...
class DatabaseSetup:
    ...
    def setup_docker_database(self) -> bool:
        ...
        try:
            ...
            subprocess.run(['docker', 'stop', 'cwe-chatbot-postgres-test'], 
                         capture_output=True)
            subprocess.run(['docker', 'rm', 'cwe-chatbot-postgres-test'], 
                         capture_output=True)
            
            # Start PostgreSQL container
            logger.info("🚀 Starting PostgreSQL + pgvector container...")
            run_result = subprocess.run([
                'docker', 'run', '--name', 'cwe-chatbot-postgres-test',
            ...
            for i in range(30):  # Wait up to 30 seconds
                try:
                    test_result = subprocess.run([
                        'docker', 'exec', 'cwe-chatbot-postgres-test',
    ...
    def setup_local_database(self) -> bool:
        ...
        try:
            ...
            subprocess.run([
                'createdb', '-h', self.config['host'], '-p', str(self.config['port']),
                '-U', self.config['user'], self.config['database']
            ], capture_output=True, env={'PGPASSWORD': self.config['password']})
    ...
    def load_test_data(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Could not import EmbeddingService - loading without embeddings")
                return self._load_test_data_no_embeddings()
            ...
            with self.connection.cursor() as cursor:
                for cwe in test_cwes:
                    ...
                    """, (
                        cwe['cwe_id'], cwe['name'], cwe['abstraction'], cwe['status'],
                        cwe['description'], cwe['extended_description'], 
                        cwe['full_text'], embedding
                    ))
    ...
    def _load_test_data_no_embeddings(self) -> bool:
        ...
        try:
            test_cwes = [
                ('CWE-79', "Cross-site Scripting", 'Base', 'Stable', 
                ...
                ('CWE-120', "Buffer Overflow", 'Base', 'Stable',
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                if embedding_count > 0:
                    ...
                    if test_record:
                        logger.info(f"✅ Database ready for vector similarity testing")
                else:
...
def main():
    parser = argparse.ArgumentParser(description='Set up PostgreSQL + pgvector database for CWE ChatBot')
    ...
    parser.add_argument('--cloud', action='store_true', help='Use cloud database (connection only)')
    parser.add_argument('--host', default='localhost', help='Database host')
    parser.add_argument('--port', type=int, default=5432, help='Database port')
    parser.add_argument('--database', default='cwe_chatbot_test', help='Database name')
    parser.add_argument('--user', default='postgres', help='Database user')
    parser.add_argument('--skip-data', action='store_true', help='Skip loading test data')
    ...
    try:
        ...
        logger.info("   poetry run pytest apps/chatbot/tests/test_real_integration.py -v")
